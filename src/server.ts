...
function rowsFromGranolaTranscript(text: string) {
    ...
    return [
      {
        ...
        atividade: `Transcricao Granola indicou ponto de avaliacao: ${question.assunto}`,
        area: question.area,
        atende,
  ...
  if (!rows.length && text.trim()) {
    rows.push({
      ...
      atividade: "Transcricao recebida do Granola sem tema automaticamente identificado.",
      area: "Compliance",
      atende: "P",
