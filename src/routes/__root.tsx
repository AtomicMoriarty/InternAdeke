...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "AdekeIntern" },
      { name: "description", content: "Adequação Interna" },
      { name: "author", content: "Lovable" },
      { property: "og:title", content: "AdekeIntern" },
      { property: "og:description", content: "Adequação Interna" },
...
function RootComponent() {
  ...
  const me = session ? profiles.find((p) => p.id === session.user.id) : null;
  const userName = me?.display_name || session?.user?.email?.split("@")[0] || "Usuário";
