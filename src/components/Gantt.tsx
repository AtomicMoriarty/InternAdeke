...
export default function Gantt() {
  ...
  return (
    <div style={{ padding: "24px 28px", fontFamily: "Outfit, sans-serif", background: "#F0F5FF", minHeight: "100vh" }}>
      {/* Cabeçalho */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 900, color: "#0F172A", letterSpacing: -0.4 }}>Linha do tempo</h1>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ display: "inline-flex", background: "#fff", borderRadius: 8, padding: 2, border: "1px solid #E2E8F0" }}>
            {["todos", ...MODULOS].map((m) => (
              <button key={m} onClick={() => setModulo(m)} style={{
                ...
                border: "none", padding: "6px 11px", borderRadius: 6,
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 8, border: `1px solid ${ocultarFinalizados ? "#0DD3C5" : "#E2E8F0"}`,
      ...
      {barras.length === 0 ? (
        ...
        <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 14, overflow: "hidden", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", borderBottom: "1px solid #E2E8F0", background: "#F8FAFC" }}>
            <div style={{ width: LARGURA_ROTULO, flexShrink: 0, padding: "8px 12px", fontSize: 10, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: 0.6 }}>
              Demanda
            </div>
            <div style={{ flex: 1, position: "relative", height: 32 }}>
              {meses.map((m) => (
                <span key={m.rotulo} style={{
                  position: "absolute", left: `${m.esquerda}%`, top: 8,
                  fontSize: 10, fontWeight: 700, color: "#64748B",
                  borderLeft: "1px solid #E2E8F0", paddingLeft: 5, height: 20,
                }}>{m.rotulo}</span>
              ))}
            </div>
          </div>
                ...
                <div key={b.item.itemId} style={{
                  ...
                  <div style={{ flex: 1, position: "relative", height: 42 }}>
                    ...
                    {posHoje >= 0 && posHoje <= 100 && (
                      <span style={{ position: "absolute", left: `${posHoje}%`, top: 0, bottom: 0, width: 1, background: "#0DD3C5", opacity: 0.45 }} />
                    )}
                    <div
                      title={`${b.item.dataInicio || "sem início"} → ${b.item.prazo || "sem prazo"}${b.fimEstimado ? " (estimado)" : ""}`}
                      style={{
                        position: "absolute", left: `${pos.esquerda}%`, width: `${pos.largura}%`,
                        top: 11, height: 20, borderRadius: 6,
                        background: b.fimEstimado
                          ? `repeating-linear-gradient(45deg, ${cor}, ${cor} 5px, ${cor}bb 5px, ${cor}bb 10px)`
                          : cor,
                        display: "flex", alignItems: "center", justifyContent: "flex-end",
                        paddingRight: 5, gap: 2, minWidth: 8,
                      }}
                    >
                      {resps.slice(0, 3).map((p, i) => (
                        <span key={p.id} title={p.display_name} style={{
                          ...
                          fontSize: 7, fontWeight: 800, display: "inline-flex",
                          alignItems: "center", justifyContent: "center",
                          border: "1px solid rgba(255,255,255,.7)", marginLeft: i ? -4 : 0,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 380, overflowY: "auto" }}>
            {semDatas.map((item) => (
              <div key={item.itemId} style={{
                ...
                background: "#F8FAFC", borderRadius: 8, border: "1px solid #F1F5F9", flexWrap: "wrap",
              }}>
                <div style={{ flex: 1, minWidth: 160 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#0F172A" }}>{item.itemNome}</div>
                  <div style={{ fontSize: 10, color: "#94A3B8" }}>{item.clienteNome} · {item.modulo}</div>
                </div>
                <CampoData
                  rotulo="Início"
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
      ...
      <input
        ...
        style={{
          width: 105, background: "#fff", border: "1px solid #E2E8F0", borderRadius: 6,
          padding: "5px 7px", fontSize: 11, fontFamily: "inherit", outline: "none", color: "#0F172A",
        }}
      />
      {atalho && !rascunho && (
        <button
          onClick={() => { setRascunho(atalho); onSalvar(atalho); }}
          title="Usar a data de hoje"
          style={{
            background: "#F1F5F9", border: "1px solid #E2E8F0", borderRadius: 6,
            padding: "5px 7px", fontSize: 10, fontWeight: 700, color: "#475569",
...
function Vazio({ titulo, texto }) {
  return (
    <div style={{ background: "#fff", border: "1px dashed #CBD5E1", borderRadius: 14, padding: 40, textAlign: "center", marginBottom: 16 }}>
