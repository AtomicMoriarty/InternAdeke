      ...
      <div ref={ref} style={{ position: "fixed", top: 16, right: 76, zIndex: 1001 }}>
        ...
        {open && (
          <Panel
            list={list}
            onClick={handleClick}
            onMarkAll={() => userId && markAllRead(userId)}
          />
        )}
      </div>
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                <h3 style={{ fontSize: 18, fontWeight: 900, color: "#0F172A" }}>
                  {userName}, você tem <span style={{ color: "#0DD3C5" }}>{unread.length}</span>{" "}
                  novidade{unread.length !== 1 ? "s" : ""}
                </h3>
              </div>
              <button
                onClick={() => setWelcomeOpen(false)}
            ...
            >
              ...
              <button onClick={() => setWelcomeOpen(false)} style={btnPrimary}>
                Fechar
              </button>
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ overflow: "auto", flex: 1 }}>
        ...
        {list.map((n) => (
          <NotifRow key={n.id} n={n} onClick={() => onClick(n)} profiles={profiles} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div
      style={{
        ...
        padding: "12px 14px",
        width: "100%",
        background: n.lida ? "transparent" : "#F0FDFA",
        borderBottom: "1px solid #F1F5F9",
        fontFamily: "inherit",
