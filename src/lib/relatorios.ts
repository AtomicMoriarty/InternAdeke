...
export type ItemPlano = {
  ...
  comentarios: { id: string; created_at?: string; date?: string; autor_id: string | null; autor_nome: string; text: string }[];
...
function meiaNoite(d: Date) { const x = new Date(d); x.setHours(0, 0, 0, 0); return x; }
...
export function achatarItens(data: any): ItemPlano[] {
  const out: ItemPlano[] = [];
...
export function tempoPorEtapa(item: ItemPlano, agora = new Date()): Record<string, number> {
  ...
  if (!hist.length) {
    const ini = item.statusChangedAt ? new Date(item.statusChangedAt)
              : item.criadoEm ? new Date(item.criadoEm)
              : null;
...
export function situacaoPrazo(item: ItemPlano, agora = new Date()): { situacao: SituacaoPrazo; dias: number | null } {
...
export function cargaPorPessoa(itens: ItemPlano[], agora = new Date()): CargaPessoa[] {
  ...
  for (const item of itens) {
    for (const uid of item.responsaveis) {
      if (!mapa.has(uid)) {
        mapa.set(uid, {
          userId: uid, total: 0, porStatus: {}, concluidos: 0,
          vencidos: 0, parados7: 0, mediaDiasEmAberto: null,
        });
        somaDias.set(uid, []);
      }
      ...
      if (situacaoPrazo(item, agora).situacao === "vencido" && item.status !== "Finalizado") c.vencidos++;
  ...
  for (const [uid, c] of mapa) {
    ...
    c.mediaDiasEmAberto = arr.length ? Math.round(arr.reduce((a, b) => a + b, 0) / arr.length) : null;
  }
  return [...mapa.values()].sort((a, b) => b.total - a.total);
}

/** Média de dias que os itens passam em cada etapa, no conjunto todo. */
export function mediaTempoPorEtapa(itens: ItemPlano[], agora = new Date()): { status: string; mediaDias: number; amostras: number }[] {
...
export function atividadeNoPeriodo(itens: ItemPlano[], desde: Date, agora = new Date()): EventoAtividade[] {
  ...
  for (const item of itens) {
    ...
    for (const c of item.comentarios) {
      ...
      ev.push({
        tipo: "comentario", em: iso!, item,
        descricao: c.text,
        autorId: c.autor_id, autorNome: c.autor_nome,
      });
    }
  }
...
export function itensParados(itens: ItemPlano[], minDias = 7, agora = new Date()): { item: ItemPlano; dias: number }[] {
  return itens
